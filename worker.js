...
const ADAPTERS = {
  ...
  */
  accounting: {
    ...
    async fetchWeekly(env, h, q) { return xeroFetchWeekly(env, h, q); }
  },
  ...
  */
  pos: {
    ...
    async fetchWeekly(env, h, q) { return squareWeekly(env, h, q); },
...
function emptyWeekly(weekStarts) {
  return { weekStarts: weekStarts, revenue: weekStarts.map(() => null), cogs: weekStarts.map(() => null), wagesSuper: weekStarts.map(() => null), overheads: weekStarts.map(() => null) };
}
...
/* Weekly trend for the Manager view: one P&L call per week, whole-week windows
   (Mon-Sun, or whatever weekStart the venue uses - the client sends the exact
   week-start dates so the server never has to guess the boundary). Deliberately
   skips the wage-week-shift double-fetch that fetchRange applies to a single
   card's WEEK view - a trend sparkline doesn't need to-the-cent wage accuracy,
   and doubling the call count here risks the Worker subrequest budget when it's
   already spending one call per week. Same simplification the monthly trend
   already makes for the same reason. */
async function xeroFetchWeekly(env, h, q) {
  const weekStarts = q.weekStarts;
  const result = {};
  weekStarts.forEach((ws) => { result[ws] = null; });
  try { await xeroTenant(env, h); } catch (e) { return emptyWeekly(weekStarts); }
  const CONC = 4;
  let idx = 0;
  const run = async () => {
    while (idx < weekStarts.length) {
      const ws = weekStarts[idx++];
      try {
        const cols = xeroParseColumns(await xeroReport(env, h, { fromDate: ws, toDate: addDaysStr(ws, 6) }));
        result[ws] = cols[0] || null;
      } catch (e) { result[ws] = null; /* one bad week never breaks the trend */ }
    }
  };
  const pool = [];
  for (let i = 0; i < Math.min(CONC, weekStarts.length); i++) pool.push(run());
  await Promise.all(pool);
  const loc = (ws, shop, field) => (result[ws] && result[ws].byLoc && result[ws].byLoc[shop]) ? result[ws].byLoc[shop][field] : null;
  return {
    weekStarts: weekStarts,
    revenue: weekStarts.map((ws) => (result[ws] ? result[ws].revenue : null)),
    cogs: weekStarts.map((ws) => (result[ws] ? result[ws].cogs : null)),
    wagesSuper: weekStarts.map((ws) => (result[ws] ? result[ws].wagesSuper : null)),
    overheads: weekStarts.map((ws) => (result[ws] ? result[ws].overheads : null)),
    revenue_windsor: weekStarts.map((ws) => loc(ws, 'windsor', 'revenue')),
    cogs_windsor: weekStarts.map((ws) => loc(ws, 'windsor', 'cogs')),
    revenue_richmond: weekStarts.map((ws) => loc(ws, 'richmond', 'revenue')),
    cogs_richmond: weekStarts.map((ws) => loc(ws, 'richmond', 'cogs'))
  };
}
...
/* Weekly trend for the Manager view - pure KV reads (the day-tally store),
   no extra Square API calls at all, so this one's cheap regardless of week count. */
async function squareWeekly(env, h, q) {
  const weekStarts = q.weekStarts;
  const allDates = [];
  const weekOf = {};
  weekStarts.forEach((ws) => {
    for (let i = 0; i < 7; i++) { const ds = addDaysStr(ws, i); allDates.push(ds); weekOf[ds] = ws; }
  });
  const { rows } = await posReadDays(env, allDates);
  const c = {}, w = {}, r = {}, has = {};
  for (const ds in rows) { const ws = weekOf[ds]; c[ws] = (c[ws] || 0) + numOr0(rows[ds].count); w[ws] = (w[ws] || 0) + numOr0(rows[ds].windsor); r[ws] = (r[ws] || 0) + numOr0(rows[ds].richmond); has[ws] = true; }
  return {
    weekStarts: weekStarts,
    count: weekStarts.map((ws) => (has[ws] ? c[ws] : null)),
    count_windsor: weekStarts.map((ws) => (has[ws] ? w[ws] : null)),
    count_richmond: weekStarts.map((ws) => (has[ws] ? r[ws] : null))
  };
}
...
async function squareSync(env, h) {
  ...
  /* Deliberate, separate capability - see the big comment above
     squareItemSalesSync(). A failure here must never break the count sync
     the main board depends on, so it's isolated in its own try/catch. */
  try { await squareItemSalesSync(env); } catch (e) { console.log('squareItemSalesSync failed: ' + (e && e.message)); }
}

async function squarePost(env, path, body) {
  const res = await fetch(SQUARE_API + path, {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + squareToken(env), Accept: 'application/json', 'Content-Type': 'application/json' },
...
/* =========================================================================
   ITEM-LEVEL SALES (Recipe costing tab only) - a DELIBERATE, NARROWER
   EXCEPTION to the "NEVER return a dollar figure from the POS" rule on the
   ADAPTERS.pos contract above. That rule protects the main KPI board:
   revenue/COGS% there must come from the accounting adapter (Xero) so they
   tie to the owner's actual books - Square's gross sales figure is
   pre-accounting (no refund netting the same way, no COGS) and would
   silently become a second, conflicting "revenue" number if it leaked into
   the board.
   This is a separate, narrower thing Kane explicitly asked for and
   approved: real per-item sales volume and revenue from Square, used ONLY
   inside the Recipe costing tab, to weight theoretical recipe cost by what
   actually sells - real (sales-mix-weighted) food cost %, popularity, and
   the what-if simulator. Never surfaced as a P&L figure anywhere else.
   Stored under its own recipeCosting:itemSales:* KV keys, fully separate
   from data:pos:* (the count-only board data these functions never touch).
   Uses Square's Orders Search API (line items on completed orders), not the
   Payments API the count-only sync above uses. */
/* v3: each per-day document now also carries byModifier (add-on / extra-patty
   / make-it-a-combo sales, which live in Square as line-item MODIFIERS and
   were previously never fetched at all). Bumping the version resets the
   backfill cursor so old v2 documents - which have byItem but no byModifier
   - get reprocessed and overwritten with the new shape over time. Readers
   must still tolerate an old document in the meantime (see
   recipeSalesRange's `|| {}` guards). */
const ITEM_SALES_SYNC_VER = 'v3';

/* Square's line items carry the CATALOG item name in li.name and, for an
   item with size/pack variations, the chosen variation in li.variation_name
   - separately. RECIPE_DATA's recipe keys are one flat descriptive string
   per size ("chicken wings 3", "say cheese large", ...), so a raw
   name.toLowerCase() key (the old v1 behaviour) can never match a
   multi-variation item - EVERY Wings/Tenders/Say Cheese/Potatoes/Fries sale
   was silently landing under a single generic bucket ("wings", "tenders", ...)
   that no recipe key could ever match, no matter which pack size sold. Same
   problem for "X promo" (kids-eat-free promo items) and a handful of items
   whose catalog name doesn't literally match its recipe key.
   VARIATION_ALIASES maps (lowercased base name, lowercased variation name)
   -> recipe key. BASE_ALIASES maps a lowercased base name (no variation, or
   after the variation lookup misses) -> recipe key, checked after a trailing
   " promo" / " - bite" suffix is stripped so promo/Bite duplicates of a
   normal item still land on the same key as the regular one. Anything not
   covered here just falls through to the old plain lowercase(name) behaviour
   unchanged, so a brand new/renamed catalog item still shows up (as
   unmatched, same as before) rather than silently vanishing. */
const VARIATION_ALIASES = {
  'wings': { '3 pack': 'chicken wings 3', '6 pack': 'chicken wings 6', '10 pack': 'chicken wings 10' },
  'tenders': { '3 pack': 'chicken tenders 3', '6 pack': 'chicken tenders 6', '10 pack': 'chicken tenders 10' },
  'say cheese': { 'small': 'say cheese small', 'large': 'say cheese large' },
  'chicken potatoes': { 'small': 'chicken potato small', 'large': 'chicken potato large' },
  'loaded vego': { 'small': 'vego potato small', 'large': 'vego potato large' },
  'cheeseburger potatoes': { 'small': 'cheeseburger potato small', 'large': 'cheeseburger potato large' },
  'spicy potatoes': { 'small': 'spicy potatoes small', 'large': 'spicy potatoes large' },
  'fries': { 'small': 'fries small', 'large': 'fries large' },
  /* "Loaded of the Month" is a rotating special like Burger of the Month -
     no fixed recipe by design, left unmapped on purpose. */
};
const BASE_ALIASES = {
  "kid's cheeseburger": 'cheese burger little ones',
  "kid's chicken burger": 'chicken burger little ones',
  "kid's tenders": 'chicken tenders little ones',
  'hefty hamburger': 'hefty burger',
  'the lot burger': 'lot burger',
  'thiccy chick burger': 'thick chick',
  'the halulu burger': 'hallulu',
  'classic daddy burger': 'classic daddy',
  'cheeky chick burger': 'cheeky chick',
  'spicy chick burger': 'spicy chick',
  'fit chick burger': 'fit chick',
  'bacon & egg': 'bacon & egg burger',
  'regular combo - burgers': 'fries and drinks',
  "regular combo - wings 'n tenders": 'fries and drinks',
  'buffalo': 'buffalo sauce',
  /* "Loaded Combo - Burgers"/"Loaded Combo - Wings 'N Tenders" are one
     generic upsell button in Square regardless of which loaded-combo flavour
     the customer actually picked - that choice lives in a MODIFIER ("Make it
     a Meal" -> "Say Cheese Loaded Fries & Drink", ...), which is now tallied
     separately into byModifier below and matched to the right per-flavour
     recipe on the dashboard side via MODIFIER_ALIASES. So these two item
     names stay unmapped here on purpose: the real flavour detail comes from
     the modifier row, and aliasing the generic item button to some single
     guessed flavour would double-count it. Same reasoning for "Cheeky Chick
     Burger Copy" (looks like a stray duplicate catalog entry, not a real
     second item) and the sauce add-ons (hot sauce, Mustard, BBQ Sauce,
     Tomato Sauce, Cheese sauce, NO EXTRA SAUCE) pending real recipes for
     those - flagged separately, not aliased here. */
};
function normalizeSquareKey(name, variationName) {
  var base = String(name).trim().toLowerCase();
  var variant = String(variationName || '').trim().toLowerCase();
  if (VARIATION_ALIASES[base] && variant && VARIATION_ALIASES[base][variant]) {
    return VARIATION_ALIASES[base][variant];
  }
  var stripped = base.replace(/\s*-\s*bite$/, '').replace(/\s+promo$/, '');
  if (BASE_ALIASES[stripped]) return BASE_ALIASES[stripped];
  if (BASE_ALIASES[base]) return BASE_ALIASES[base];
  /* No alias/recipe hit - still return the promo/Bite-stripped form (not the
     raw base) so "X", "X promo" and "X - BITE" land on the same unmatched
     bucket together instead of splitting into three separately-invisible
     ones. */
  return stripped;
}

/* Modifiers have no variation concept (Square's OrderLineItemModifier is just
   {uid, catalog_object_id, name, quantity, base_price_money, total_price_money}
   - notably NO modifier-list name), so the key is simply the trimmed,
   lowercased option name. No alias table lives here on purpose: modifier ->
   recipe aliasing happens dashboard-side in MODIFIER_ALIASES, mirroring how
   SALES_ALIASES (not BASE_ALIASES) is where Kane's own item matches live, so
   he can re-map one without a code change and a redeploy. */
function normalizeModifierKey(name) {
  return String(name == null ? '' : name).trim().toLowerCase();
}

/* Tally per-day, per-item-name sales for a batch of Sydney dates. Square
   Orders Search wants one location_id per call, so each active location is
   queried separately over the same window and results are split by shop. */
async function squareItemSalesTallyDates(env, dates, pageCap) {
  if (!dates.length) return;
  const want = new Set(dates);
  const sorted = dates.slice().sort();
  const beginUtc = addDaysStr(sorted[0], -1) + 'T00:00:00Z';
  const endUtc = addDaysStr(sorted[sorted.length - 1], 2) + 'T00:00:00Z';
  const byDate = {}; /* date -> itemNameLower -> {name, qty, gross, windsor:{qty,gross}, richmond:{qty,gross}} */
  const modByDate = {}; /* date -> modifierNameLower -> same shape, plus an optional `list` label */
  for (const d of dates) { byDate[d] = {}; modByDate[d] = {}; }
  const active = (await squareLocations(env)).filter((l) => (l.status || 'ACTIVE') === 'ACTIVE');
  for (const locObj of active) {
    const locId = locObj.id;
    if (!locId) continue; /* Orders Search requires a location_id */
    const slug = locSlug(locObj.name || '');
    let cursor = null, pages = 0;
    do {
      const body = {
        ...
        query: {
          filter: {
            date_time_filter: { closed_at: { start_at: beginUtc, end_at: endUtc } },
      ...
      const data = await squarePost(env, '/v2/orders/search', body);
      for (const order of (data.orders || [])) {
        const d = sydDate(order.closed_at || order.created_at);
        if (!want.has(d)) continue;
        const bucket = byDate[d];
        const modBucket = modByDate[d];
        for (const li of (order.line_items || [])) {
          const name = (li.name || '').trim();
          if (!name) continue;
          const key = normalizeSquareKey(name, li.variation_name);
          const qty = parseFloat(li.quantity) || 0;
          const gross = li.gross_sales_money ? (li.gross_sales_money.amount || 0) / 100 : 0;
          if (!bucket[key]) bucket[key] = { name: name, qty: 0, gross: 0, windsor: { qty: 0, gross: 0 }, richmond: { qty: 0, gross: 0 } };
          bucket[key].qty += qty;
          bucket[key].gross += gross;
          if (slug === 'windsor') { bucket[key].windsor.qty += qty; bucket[key].windsor.gross += gross; }
          else if (slug === 'richmond') { bucket[key].richmond.qty += qty; bucket[key].richmond.gross += gross; }

          /* ---- Modifiers (add-ons, extra patty, make-it-a-combo) ----
             Several genuinely revenue-bearing things at this venue are NOT
             separate catalog items in Square - they're modifiers attached to
             a line item: the "Add-ons" list (ADD BACON, ADD PINEAPPLE, ...),
             the "Extra Patty" list (double/triple Patty) and the "Make it a
             Meal" combo upsell. Before v3 these were never fetched at all -
             not even surfaced as unmatched - so their revenue was invisible
             to the sales-mix report.

             *** REVENUE-ADDITIVITY ASSUMPTION - the one thing that would
             silently corrupt Kane's numbers if it were wrong ***
             Square prices and reports a line item's modifiers INDEPENDENTLY
             of the item: li.gross_sales_money (what byItem sums above) is
             the base item's own price x quantity and does NOT include any
             modifier's price. So byModifier gross is disjoint from byItem
             gross, and the two are summed ADDITIVELY downstream (sales-mix
             revenue = item gross + modifier gross). Never substitute one for
             the other, and never subtract modifier gross out of item gross
             "to avoid double counting" - there is no double count to undo,
             and doing so would understate revenue by exactly the add-on
             take. Verified against the live Square account's order payloads.

             total_price_money already accounts for the modifier quantity, so
             it's used as-is when present; base_price_money is per-unit and is
             multiplied by quantity. A modifier with no price money at all is
             a free choice (Standard Patty, NO TOMATO, a bun choice) - it
             still tallies qty, at gross 0, because "how many double patties
             went out" is worth knowing even when it isn't worth money. */
          for (const mod of (li.modifiers || [])) {
            const mName = (mod.name || '').trim();
            if (!mName) continue;
            const mKey = normalizeModifierKey(mName);
            /* A modifier's own quantity is per line item; when Square omits
               it (common - it's implicitly 1 per item) fall back to the line
               item's quantity so 2x "Classic Daddy + ADD BACON" counts 2
               bacon add-ons, not 1. */
            const mQtyRaw = parseFloat(mod.quantity);
            const mQty = isFinite(mQtyRaw) && mQtyRaw > 0 ? mQtyRaw : qty;
            let mGross = 0;
            if (mod.total_price_money && typeof mod.total_price_money.amount === 'number') {
              mGross = mod.total_price_money.amount / 100;
            } else if (mod.base_price_money && typeof mod.base_price_money.amount === 'number') {
              mGross = (mod.base_price_money.amount / 100) * mQty;
            }
            if (!modBucket[mKey]) modBucket[mKey] = { name: mName, list: null, qty: 0, gross: 0, windsor: { qty: 0, gross: 0 }, richmond: { qty: 0, gross: 0 } };
            /* Square's order payload doesn't carry the modifier LIST name
               today; kept as a pass-through so that if it ever appears (or a
               future catalog-enriched sync fills it in) the dashboard's
               "...Removal" auto-ignore can key off it rather than guessing
               from the option name alone. */
            if (mod.modifier_list_name && !modBucket[mKey].list) modBucket[mKey].list = String(mod.modifier_list_name);
            modBucket[mKey].qty += mQty;
            modBucket[mKey].gross += mGross;
            if (slug === 'windsor') { modBucket[mKey].windsor.qty += mQty; modBucket[mKey].windsor.gross += mGross; }
            else if (slug === 'richmond') { modBucket[mKey].richmond.qty += mQty; modBucket[mKey].richmond.gross += mGross; }
          }
        }
      }
      cursor = data.cursor || null;
  ...
  await Promise.all(dates.map((d) => env.TOKENS.put('recipeCosting:itemSales:' + d, JSON.stringify({ byItem: byDate[d], byModifier: modByDate[d] }))));
}

/* Same recent-days-fresh + chunked-backfill shape as squareSync, on its own
   cursor so it can run independently. 400 days (~13 months) is plenty for
   any weekly/monthly sales-mix trend the costing tab needs, and keeps the
   backfill from running forever on first connect. */
async function squareItemSalesSync(env) {
  if (!squareToken(env)) return;
  const today = sydDate(new Date().toISOString());
  if ((await env.TOKENS.get('recipeCosting:itemSales:sync_ver')) !== ITEM_SALES_SYNC_VER) {
    await env.TOKENS.put('recipeCosting:itemSales:backfill_cursor', addDaysStr(today, 1));
    await env.TOKENS.put('recipeCosting:itemSales:sync_ver', ITEM_SALES_SYNC_VER);
  }
  await squareItemSalesTallyDates(env, lastNDates(today, 3), 40);
  const floor = addDaysStr(today, -400);
  const anchor = (await env.TOKENS.get('recipeCosting:itemSales:backfill_cursor')) || addDaysStr(today, 1);
  if (anchor > floor) {
    const chunk = [];
    let d = addDaysStr(anchor, -1);
    for (let i = 0; i < 14 && d >= floor; i++) { chunk.push(d); d = addDaysStr(d, -1); }
    if (chunk.length) {
      await squareItemSalesTallyDates(env, chunk, 40);
      await env.TOKENS.put('recipeCosting:itemSales:backfill_cursor', chunk[chunk.length - 1]);
    }
  }
}

/* Read+merge per-day item sales for a date range - same parallel-batched KV
   read pattern as posReadDays. */
async function recipeSalesRange(env, from, to) {
  ...
  for (let i = 0; i < dates.length; i += CH) {
    const batch = dates.slice(i, i + CH);
    const vals = await Promise.all(batch.map((d) => env.TOKENS.get('recipeCosting:itemSales:' + d)));
    for (let j = 0; j < batch.length; j++) {
      if (vals[j] == null) continue;
      days++;
      let day;
      try { day = JSON.parse(vals[j]); } catch (e) { continue; }
      for (const key in (day.byItem || {})) {
        const row = day.byItem[key];
        if (!byItem[key]) byItem[key] = { name: row.name, qty: 0, gross: 0, windsor: { qty: 0, gross: 0 }, richmond: { qty: 0, gross: 0 } };
        byItem[key].name = row.name || byItem[key].name;
        byItem[key].qty += numOr0(row.qty);
        byItem[key].gross += numOr0(row.gross);
        byItem[key].windsor.qty += numOr0(row.windsor && row.windsor.qty);
        byItem[key].windsor.gross += numOr0(row.windsor && row.windsor.gross);
        byItem[key].richmond.qty += numOr0(row.richmond && row.richmond.qty);
        byItem[key].richmond.gross += numOr0(row.richmond && row.richmond.gross);
      }
      /* `day.byModifier || {}` matters for real, not just defensively: every
         per-day document written before ITEM_SALES_SYNC_VER v3 has byItem and
         no byModifier at all, and those sit in KV until the reset backfill
         cursor walks back over them. An old document must read as "no
         modifier sales that day", never throw. */
      for (const key in (day.byModifier || {})) {
        const row = day.byModifier[key];
        if (!byModifier[key]) byModifier[key] = { name: row.name, list: (row.list || null), qty: 0, gross: 0, windsor: { qty: 0, gross: 0 }, richmond: { qty: 0, gross: 0 } };
        byModifier[key].name = row.name || byModifier[key].name;
        if (row.list && !byModifier[key].list) byModifier[key].list = row.list;
        byModifier[key].qty += numOr0(row.qty);
        byModifier[key].gross += numOr0(row.gross);
        byModifier[key].windsor.qty += numOr0(row.windsor && row.windsor.qty);
        byModifier[key].windsor.gross += numOr0(row.windsor && row.windsor.gross);
        byModifier[key].richmond.qty += numOr0(row.richmond && row.richmond.qty);
        byModifier[key].richmond.gross += numOr0(row.richmond && row.richmond.gross);
      }
    }
  }
  return { days, byItem, byModifier };
}

async function apiRecipeSalesGet(env, url) {
  const to = url.searchParams.get('to') || sydDate(new Date().toISOString());
  const daysParam = parseInt(url.searchParams.get('days') || '', 10);
  const span = (isFinite(daysParam) && daysParam > 0) ? daysParam : 84; /* ~12 weeks default */
  const from = url.searchParams.get('from') || addDaysStr(to, -(span - 1));
  const { days, byItem, byModifier } = await recipeSalesRange(env, from, to);
  const backfillCursor = await env.TOKENS.get('recipeCosting:itemSales:backfill_cursor');
  /* byModifier is a SEPARATE bucket from byItem, not a subset of it - see the
     revenue-additivity note in squareItemSalesTallyDates. The dashboard folds
     both into one per-recipe sales-mix aggregate by adding them. */
  return json({ from: from, to: to, days: days, byItem: byItem, byModifier: byModifier, backfillCursor: backfillCursor || null, connected: !!squareToken(env) });
}
...
/* Weekly trend request: an explicit comma-separated list of week-start dates
   (ascending), sent by the client rather than generated server-side, since only
   the client knows the venue's weekStart setting - the server just fetches
   whatever whole weeks it's given. Capped small; this is a sparkline's worth of
   history, not a full trend range. */
function parseWeekList(s) {
  if (!s) return null;
  const parts = s.split(',').filter(Boolean);
  if (!parts.length || parts.length > 16) return null;
  for (const p of parts) if (!/^\d{4}-\d{2}-\d{2}$/.test(p)) return null;
  return parts;
}
...
async function fetchSlot(env, q) {
  ...
  for (const source of ['accounting', 'pos', 'rostering']) {
    ...
    /* noteSync is best-effort bookkeeping (the "last synced" timestamp) only -
       it must never wipe out an otherwise-successful fetch above. A single
       /api/metrics request calls this up to 3x per source in quick succession
       (cur/prev/yoy), which can trip KV's one-write-per-second-per-key limit;
       that failure used to land in the same try/catch as the real fetch and
       null out good data. Isolated here so it can never do that again. */
    try { await noteSync(env, source); } catch (e) {}
  }
  return out;
}
...
async function apiMetrics(env, url) {
  ...
  const trendW = parseWeekList(url.searchParams.get('trendW'));
  ...
  const cacheKey = 'metricscache:' + [
    ...
    url.searchParams.get('trendW') || '', tz, rollover
  ].join('|');
  ...
  if (!data) {
    ...
    let trendWOut = null;
    if (trendW) {
      trendWOut = { weekStarts: trendW };
      for (const source of ['accounting', 'pos']) {
        const adapter = ADAPTERS[source];
        if (!adapter || !adapter.configured || typeof adapter.fetchWeekly !== 'function') { trendWOut[source] = null; continue; }
        try {
          const h = makeHelpers(env, source);
          const series = await adapter.fetchWeekly(env, h, { ...base, weekStarts: trendW });
          trendWOut[source] = alignSeriesWeekly(trendWOut.weekStarts, series);
        } catch (err) { trendWOut[source] = null; }
      }
    }
    data = { generatedAt: new Date().toISOString(), periods: periods, trend: trendOut, trendW: trendWOut };
    if (env.TOKENS) {
...
/* Same idea, keyed by week-start date instead of month. */
function alignSeriesWeekly(weekStarts, series) {
  if (!series || !Array.isArray(series.weekStarts)) return null;
  const idx = {};
  series.weekStarts.forEach((ws, i) => { idx[ws] = i; });
  const out = {};
  Object.keys(series).forEach((k) => {
    if (k === 'weekStarts') return;
    out[k] = weekStarts.map((ws) => (ws in idx && series[k] ? (series[k][idx[ws]] ?? null) : null));
  });
  return out;
}

/* ---------------- Recipe costing: ingredient price overrides ----------------
   The recipe STRUCTURE (61 menu items, their ingredient lines and quantities)
   is a static JS constant baked into dashboard.html - it barely changes and a
   redeploy is fine when it does. Ingredient PRICES change every week (the
   "gmail suppliers" chat re-checks them), so they live here in KV instead,
   under one fixed key, and dashboard.html fetches them at runtime and merges
   them over its own baked-in defaults - no redeploy needed for a price update.

   POST /api/ingredient-prices  (same Bearer INGEST_TOKEN as /api/ingest)
     Body: JSON object mapping ingredient name (lowercase, matching the name
     used in dashboard.html's RECIPE_RAW) -> { price: number, note?: string }.
     A partial update is fine - only the keys sent are changed; anything else
     already stored (or any ingredient dashboard.html has no override for)
     keeps its previous/baked-in value. Send { "_replace": true, ...prices }
     to replace the whole stored set instead of merging.
   GET  /api/ingredient-prices  (requires the dashboard's own login, like
     /api/metrics - this is the owner's cost data, not for public fetch)
     Returns whatever's currently stored: { updatedAt, prices: {...} }. An
     empty {prices: {}} is normal before the first weekly update is posted -
     dashboard.html just keeps using its baked-in defaults for everything. */
const INGREDIENT_PRICES_KEY = 'recipeCosting:ingredientPrices';

async function apiIngredientPricesGet(env) {
  const raw = await env.TOKENS.get(INGREDIENT_PRICES_KEY);
  if (!raw) return json({ updatedAt: null, prices: {} });
  try { return json(JSON.parse(raw)); } catch (e) { return json({ updatedAt: null, prices: {} }); }
}

async function apiIngredientPricesPost(env, request) {
  const auth = request.headers.get('Authorization') || '';
  if (!env.INGEST_TOKEN || auth !== 'Bearer ' + env.INGEST_TOKEN) {
    return json({ error: 'not authorised', plain: 'That upload code didn’t match. Check it with your AI and try again.' }, 401);
  }
  let body;
  try { body = await request.json(); } catch (e) { return json({ error: 'bad json', plain: 'That wasn’t valid JSON.' }, 400); }
  ...
  for (const k of Object.keys(body)) {
    ...
    incoming[k.trim().toLowerCase()] = { price, note: (v && typeof v === 'object' && v.note) ? String(v.note) : undefined };
  }
  let merged = incoming;
  if (!replace) {
    const raw = await env.TOKENS.get(INGREDIENT_PRICES_KEY);
    let existing = {};
    if (raw) { try { existing = JSON.parse(raw).prices || {}; } catch (e) {} }
    merged = Object.assign({}, existing, incoming);
  }
  const payload = { updatedAt: new Date().toISOString(), prices: merged };
  await env.TOKENS.put(INGREDIENT_PRICES_KEY, JSON.stringify(payload));
  return json({ ok: true, count: Object.keys(incoming).length, total: Object.keys(merged).length });
}

/* ---------------- Recipe costing: recipe/ingredient library edits ----------------
   Kane edits recipes and the raw ingredient library live from the dashboard
   (resize a portion, drop an ingredient, add a whole new menu item) - this is
   an interactive edit from the logged-in owner, not a weekly bot upload, so
   it's protected by the normal dashboard session cookie (isLoggedIn), same as
   /api/metrics, rather than the INGEST_TOKEN used by the weekly price feed.

   Stores the WHOLE {raw, recipes} structure as one JSON document - small
   enough (well under KV's per-value limit) that there's no value in a more
   granular scheme, and it means a save can never leave the two halves
   inconsistent with each other.

   GET  /api/recipes  -> { updatedAt, data: {raw, recipes} | null }. null
     means nothing has been saved yet - dashboard.html keeps using its own
     baked-in RECIPE_DATA until the first save.
   POST /api/recipes  body: { raw: {...}, recipes: {...} } - replaces the
     whole stored document. dashboard.html validates ingredient references
     client-side before sending (every line must resolve to a raw ingredient
     or another recipe), but re-checked here too since this sets what every
     future load of the dashboard computes cost from. */
const RECIPES_KEY = 'recipeCosting:recipes';

async function apiRecipesGet(env) {
  const raw = await env.TOKENS.get(RECIPES_KEY);
  if (!raw) return json({ updatedAt: null, data: null });
  try { return json(JSON.parse(raw)); } catch (e) { return json({ updatedAt: null, data: null }); }
}

function validateRecipeDoc(doc) {
  if (!doc || typeof doc !== 'object') return 'body must be an object';
  if (!doc.raw || typeof doc.raw !== 'object' || Array.isArray(doc.raw)) return 'missing "raw" ingredient map';
  if (!doc.recipes || typeof doc.recipes !== 'object' || Array.isArray(doc.recipes)) return 'missing "recipes" map';
  for (const key of Object.keys(doc.recipes)) {
    const rec = doc.recipes[key];
    if (!rec || !Array.isArray(rec.lines)) return 'recipe "' + key + '" has no lines array';
    for (const line of rec.lines) {
      if (!line || typeof line.ingredient !== 'string' || typeof line.qty !== 'number') {
        return 'recipe "' + key + '" has a malformed ingredient line';
      }
      /* A name existing as BOTH a raw ingredient and a recipe (bacon, cheese
         slice, etc. - a standalone add-on wrapping a same-named raw
         ingredient) always resolves to the raw row, wherever it's
         referenced - never through the recipe of the same name, which
         would double the recipe's own baked-in portion size on top of
         whatever quantity THIS line specifies. See the matching comment on
         dashboard.html's inferLineKind() for the full explanation. */
      const iname = line.ingredient.trim().toLowerCase();
      if (!doc.raw[iname] && !doc.recipes[iname]) return 'recipe "' + key + '" references unknown ingredient "' + line.ingredient + '"';
    }
  }
  return null;
}

async function apiRecipesPost(env, request) {
  if (!(await isLoggedIn(request, env))) return json({ error: 'auth' }, 401);
  let body;
  try { body = await request.json(); } catch (e) { return json({ error: 'bad json', plain: 'That wasn’t valid JSON.' }, 400); }
  const problem = validateRecipeDoc(body);
  if (problem) return json({ error: 'invalid', plain: problem }, 422);
  const payload = { updatedAt: new Date().toISOString(), data: { raw: body.raw, recipes: body.recipes } };
  await env.TOKENS.put(RECIPES_KEY, JSON.stringify(payload));
  return json({ ok: true, updatedAt: payload.updatedAt });
}

/* ---------------- App state: everything Kane types into the dashboard ----------------
   Overrides (typed Revenue/COGS/Discounts/Uber per period), Settings (targets,
   venue name, view mode, data mode...), Recon confirmations, connected-org
   confirmations, and full Manual-mode entries all used to live ONLY in the
   browser's localStorage - meaning none of it followed Kane from his laptop
   to his phone, or to a different browser. This mirrors the /api/recipes
   pattern: one JSON document, one KV key, protected by the normal session
   cookie. dashboard.html keeps a localStorage copy too, purely as an instant-
   paint cache and an offline fallback if a save can't reach the server -
   THIS is the source of truth now, not the browser.

   GET  /api/state  -> { updatedAt, data: {...} | null }. null means nothing
     has been saved to the server yet (e.g. first load after this feature
     shipped) - dashboard.html falls back to whatever it has in localStorage
     from before, then pushes it up with the next save.
   POST /api/state   body: { settings, recon, orgOk, manual, overrides, uber }
     - replaces the whole stored document. dashboard.html debounces rapid
     edits into one save rather than one KV write per keystroke, to stay
     well clear of the daily KV write quota. */
const APPSTATE_KEY = 'appstate:v1';

async function apiAppStateGet(env) {
  const raw = await env.TOKENS.get(APPSTATE_KEY);
  if (!raw) return json({ updatedAt: null, data: null });
  try { return json(JSON.parse(raw)); } catch (e) { return json({ updatedAt: null, data: null }); }
}

function validateAppState(doc) {
  if (!doc || typeof doc !== 'object' || Array.isArray(doc)) return 'body must be an object';
  const fields = ['settings', 'recon', 'orgOk', 'manual', 'overrides', 'uber', 'salesAliases', 'modifierAliases', 'priceSeen'];
  for (const f of fields) {
    if (f in doc && (typeof doc[f] !== 'object' || doc[f] === null || Array.isArray(doc[f]))) {
...
async function apiAppStatePost(env, request) {
  if (!(await isLoggedIn(request, env))) return json({ error: 'auth' }, 401);
  let body;
  try { body = await request.json(); } catch (e) { return json({ error: 'bad json', plain: 'That wasn’t valid JSON.' }, 400); }
  const problem = validateAppState(body);
  if (problem) return json({ error: 'invalid', plain: problem }, 422);
  const payload = {
    ...
    data: {
      ...
      uber: body.uber || {},
      /* salesAliases: Square-sold item name -> recipe key (or '__ignore__'),
         from the Recipe costing tab's "match Square sales to menu items"
         editor - lets Kane fix a naming mismatch from the dashboard instead
         of needing a code change + redeploy. priceSeen: ingredient key ->
         the price the dashboard last showed Kane, so it can flag "COGS
         moved because a supplier price changed" only once per change.
         modifierAliases: the same thing for Square MODIFIER names (add-ons,
         extra patty, make-it-a-combo) - kept in its own map rather than
         merged into salesAliases because a modifier and an item can share a
         name and must be able to point at different recipes. */
      salesAliases: body.salesAliases || {},
      modifierAliases: body.modifierAliases || {},
      priceSeen: body.priceSeen || {}
    }
  };
  await env.TOKENS.put(APPSTATE_KEY, JSON.stringify(payload));
  return json({ ok: true, updatedAt: payload.updatedAt });
}
...
async function handleFetch(request, env) {
    ...
    if (path === '/api/ingredient-prices' && request.method === 'POST') return apiIngredientPricesPost(env, request);
    if (path === '/api/state' && request.method === 'POST') return apiAppStatePost(env, request);
    ...
    if (path === '/api/ingredient-prices' && request.method === 'GET') {
      if (!loggedIn) return json({ error: 'auth' }, 401);
      return apiIngredientPricesGet(env);
    }
    if (path === '/api/recipes' && request.method === 'GET') {
      if (!loggedIn) return json({ error: 'auth' }, 401);
      return apiRecipesGet(env);
    }
    if (path === '/api/recipes' && request.method === 'POST') return apiRecipesPost(env, request);
    if (path === '/api/state' && request.method === 'GET') {
      if (!loggedIn) return json({ error: 'auth' }, 401);
      return apiAppStateGet(env);
    }
    if (path === '/api/recipe-sales' && request.method === 'GET') {
      if (!loggedIn) return json({ error: 'auth' }, 401);
      return apiRecipeSalesGet(env, url);
    }
...
export default {
  async fetch(request, env) {
    try {
      ...
      /* No safety net previously existed here, so any uncaught exception
         anywhere in routing surfaced as Cloudflare's bare "Error 1101"
         page with zero detail - impossible to diagnose without Workers
         Logs. This always returns the real error message instead. */
      return new Response(
  ...
  async scheduled(event, env, ctx) { return handleScheduled(event, env, ctx); },
  async email(message, env, ctx) { return handleEmail(message, env, ctx); }
};
// EOF worker.js
