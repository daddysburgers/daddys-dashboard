...
const BASE_ALIASES = {
  ...
  /* "Loaded Combo - Burgers"/"Loaded Combo - Wings 'N Tenders" are one
     ...
     the customer actually picked (that choice lives in a modifier, not the
     item name) - left unmapped rather than guessed. Same for "Cheeky Chick
...
async function squareItemSalesTallyDates(env, dates, pageCap) {
  ...
  await Promise.all(dates.map((d) => env.TOKENS.put('recipeCosting:itemSales:' + d, JSON.stringify({ byItem: byDate[d] }))));
}
...
async function apiRecipeSalesGet(env, url) {
  ...
  const { days, byItem } = await recipeSalesRange(env, from, to);
  const backfillCursor = await env.TOKENS.get('recipeCosting:itemSales:backfill_cursor');
  return json({ from: from, to: to, days: days, byItem: byItem, backfillCursor: backfillCursor || null, connected: !!squareToken(env) });
}
...
function validateAppState(doc) {
  ...
  const fields = ['settings', 'recon', 'orgOk', 'manual', 'overrides', 'uber'];
